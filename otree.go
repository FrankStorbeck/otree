...
// node in the tree. If the parent is not a node in the tree ErrNodeNotFound
// will be returned. The children with all of its descendants will be inserted
// just before the sibling with index i. However if i is negative they wil be
// inserted before the first sibling. If the index is larger than the index of
// the last sibling they will be appended to the end of the siblings.
// If one of the children or one of their descendants are already part of they
// tree ErrDuplicateNodeFound will be returned.
func (tr *Tree) LinkChildren(parent *Node, i int, children ...*Node) error {
	...
	for _, nd := range children {
		var found bool
		nd.Walk(nil, func(nd *Node, data interface{}) {
			if !found {
				_, found = tr.present[nd]
			}
		})
		if found {
